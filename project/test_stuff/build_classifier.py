...
from sklearn.metrics import precision_recall_curve, recall_score, auc, f1_score, roc_curve
import pylab as pl
...
MODELS_TO_RUN =  ['LR', 'DT', 'NB','KNN', 'AB', 'RF','ET', 'SVM', 'GB']
K_FOLDS = 5
THRESHOLDS = [x/10 for x in range(10)][1::2]
TOP_MODELS_FILENAME = 'top_models.csv'
TOP_N = 30
    ...
    for index, clf in enumerate([clfs[x] for x in models_to_run]):
        ...
        for p in ParameterGrid(parameter_values):
            ...
            for num_fold in range(K_folds):
                X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state = num_fold)
                clf.fit(X_train, y_train)
                y_pred_probs = clf.predict_proba(X_test)[:,1]
                auprc_val, auroc_val = mine.get_eval_stats(y_test, y_pred_probs)
...
def rebuild_and_get_point_metrics(X, y, top_models, clfs = CLFS, thresholds = THRESHOLDS, plot = False):
    ...
    for index, row in top_models.iterrows():
        ...
        for thresh in thresholds:
            ...
            row_of_metrics += mine.get_model_evaluation_stats(y_test, y_pred)
        if plot:
...
# -----------------------------------------------------------------------
# ---------------- HELPER FUNCTIONS FROM MY_LIBRARY.PY ------------------
# -----------------------------------------------------------------------
